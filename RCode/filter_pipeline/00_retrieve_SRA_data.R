...
fasp = listSRAfile(
  rs$run_accession,sra_con = sra_con,fileType = "fastq",srcType = "fasp"
)

## Download sra FASTQ file using ASPERA protocol:
ascpCMD <-
  "/usr/local/bioinfo/src/Aspera/current/bin/ascp -i /usr/local/bioinfo/src/Aspera/current/etc/asperaweb_id_dsa.openssh -QT -l 500m"
